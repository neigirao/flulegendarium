...
test.describe('Gameplay - Quiz de Camisas', () => {
  ...
  test('should display year options for selection', async ({ page }) => {
    ...
    const yearOptions = page.locator('button:has-text(/19[0-9]{2}|20[0-9]{2}/)');
  ...
  test('should allow selecting a year option', async ({ page }) => {
    ...
    // Após clique, algo deve acontecer (feedback visual, próxima camisa, ou dialog)
    const selectedButton = page.locator('button[class*="selected"], button[aria-pressed="true"]');
    const resultIndicator = page.locator('text=/correto|errado|certo|incorreto/i');
  ...
  test('should show visual feedback for correct answer', async ({ page }) => {
    ...
    // Deve haver algum tipo de feedback visual (cor verde/vermelha, ícone, etc)
    const correctIndicator = page.locator('[class*="green"], [class*="success"], .bg-green, text=/✓|correto|certo/i');
    const incorrectIndicator = page.locator('[class*="red"], [class*="error"], [class*="destructive"], .bg-red, text=/✗|incorreto|errado/i');
    
    const hasCorrect = await correctIndicator.count() > 0;
    const hasIncorrect = await incorrectIndicator.count() > 0;
    
    // Pelo menos um tipo de feedback deve aparecer
  ...
  test('should show score after selecting answer', async ({ page }) => {
    ...
    const scoreDisplay = page.locator('text=/pontos?|score|pts/i');
  ...
  test('should show game over after incorrect answer', async ({ page }) => {
    ...
    const gameOverDialog = page.locator('[role="dialog"], text=/game over|fim|errou/i');
    const nextJersey = page.locator('[data-testid="jersey-image"]');
    
    const hasGameOver = await gameOverDialog.count() > 0;
    const hasNextJersey = await nextJersey.count() > 0;
    
    // Deve ter resultado: game over ou próxima camisa
  ...
  test('should have skip player button', async ({ page }) => {
    ...
    const skipButton = page.locator('button:has-text(/pular|skip/i)');
